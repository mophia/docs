...
有的DBA或许会认为 UNDO 是 REDO 的逆过程，其实不然。REDO 和 UNDO 都可以视为是一种恢复墚作，但是：

- redo log：是存储引擎层(innodb)生成的日志，记录的是"物理级别“上的页修改操作，比如页号xx、偏移量yy
  写人了 zzz数据。主要为了保证数据的可靠性：
- undo log：是存储引擎层(innodb)生成的日志，记录的是 迎辑探作 日志，比如对某一行数据进行了INSERT语句
  操作，那么undo log就记录一条与之相反的DELETE操作。主要用于 事务的回滚(undo 10g 记录的是每个修改操
  作的近操作)和一致性非锁定读(undo log 回滚行记录到某种特定的版本--MVCC，即多版本并发控制)。



## 1. redo日志

InnoDB存储引擎是以 页为单位 来管理存储空问的。在真正访问页面之前，需要把在 磁盘上的页缓存到内存中的Buffer Pool之后才可以访问。所有的变更都心须 先**更新缓冲池**中的数据，然后缓冲池中的胜页会以一定的频率被刷入磁盘（checkPoint机制），通过缓冲池来优化CPU和磁盘之间的鸿沟，这样就可以保证整体的性能不会下降太快。
...
- 设置为1:表示每次事务提交时都只把 redo log buffer 内容写入 page cache，不进行同步。由os自
...
另外，InnoDe 存储号引擎有一个后台线程，每隔，1秒，就会把redo 1og butfer 中的内容写到文件系统缓存( page cache），然后调用刷盘操作。
...
> innodb_flushllog_at_trx_ commit=l
> 为1时，只要事务提交成功，redo1og 记录就一定在硬盘里，不会有任何数据丢失。
> 如果事务执行期间 MysQL挂了或宕机，这部分日志丢了，但是事务并没有提交，所以日志丢了也不会有损
> 失。可以保证ACID的D，数据绝对不会丢失，但是效率最差的。
> 建议使用默认值，星然操作系统宕机的概率理论小于数据库宕机的概率，但是一股既然使用了事务，那么数
> 据的安全相对来说更重要些
...
> 如果仅仅只是 MysQL挂了不会有任何数据丢失，但是操作系统宏机可能会有7秒数据的丢失，这种情况下无
